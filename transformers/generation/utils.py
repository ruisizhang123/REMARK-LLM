    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if 'message' in list(model_kwargs.keys()):
                outputs = self(
                    **model_inputs,
                    message=model_kwargs['message'],
                    message_embed_method = model_kwargs['message_embed_method'],
                    return_dict=True,
            ...
            # add noise to next_tokens_scores
            next_tokens_scores = torch.nn.functional.gumbel_softmax(next_tokens_scores, tau=0.5, hard=True)
            # Store scores, attentions and hidden_states when required
            if return_dict_in_generate:
                if output_scores:
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            #probs = (probs+1e-20)
            probs = F.gumbel_softmax(next_token_scores, tau=0.5, eps=1e-10, dim=-1)
            #import pdb; pdb.set_trace()
            next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1)
            # if 0 in next_tokens, resample
            #while 0 in next_tokens:
            #    next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1)
            ...
            if this_peer_finished and not synced_gpus:
                break
            if input_ids.size(1) == model_kwargs['attention_mask'].size(1):
                break
        #
        if streamer is not None:
            streamer.end()
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            #import pdb; pdb.set_trace()
            #logit = F.gumbel_softmax(logit, tau=0.5, eps=1e-10, dim=-1)
            ...
            #next_token_scores = F.gumbel_softmax(next_token_scores, tau=0.5, eps=1e-10, dim=-1)
            #import pdb; pdb.set_trace()
            ...
            # get concated next tokens
            #import pdb; pdb.set_trace()
            '''
            updated_token = torch.cat([input_ids, next_tokens], dim=1)
            dist = F.one_hot(updated_token, num_classes=extra_config['vocab_size']).float()
            outputs_ebd = mapper(dist)
            wm_logits = extractor(outputs_ebd)
            next_token_scores = next_token_scores + nn.L1Loss()(wm_logits, model_kwargs['message'])
            '''
